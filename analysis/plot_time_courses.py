...
test_words = ['climate', 'environment', 'genetics', 'computer', 'war', 'genocide', 'philanthropy', 'donation',
              'gender', 'feminism', 'racism', 'automation', 'water']
...
reduced_mfd_dict_null = mfd_dict_null[mfd_dict_null[constant.YEAR] == mfd_year]
for c in all_models:
    ...
    for i,word in enumerate(test_words):
        ...
        if 'FINEGRAINED' == binary_fine_grained:
            ...
            if binary_fine_grained == 'BINARY':
                cat, dir, color = '+', 'binaryseparate', 'blue'
            else:
                cat, dir, color = '1', 'null', 'black'
            ...
            plt.xlabel('Years')
            if plot_separate:
                plt.plot(word_df[constant.YEAR].values, cat_prediction, label='Moral Polarity', linewidth=2.0, color=color)
                plt.fill_between(word_df[constant.YEAR].values, cat_prediction_l, cat_prediction_u, alpha=0.2, color=color)
                plt.legend()
                # plt.ylim(0.45,0.55)
                # plt.hlines(0, min(word_df[constant.YEAR].values), max(word_df[constant.YEAR].values), colors='grey')
