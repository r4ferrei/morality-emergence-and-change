...
def plot_binary(word, mean_line, head_word, years, btstrap, binary_fine_grained, ylim1, ylim2, xlim1, yticks,
    ...
    if btstrap:
        plt.fill_between(years, cat_prediction_l,
                         cat_prediction_u, alpha=0.2, color=color)
...
def plot_category_hinton(categories, preds, years_locs, max_weight, min_weight):
    ...
    for x in years_locs:
        ax.text(x - 1 / 2, -2,
                constant.ALL_YEARS[x], horizontalalignment='center')
    for y, cat in enumerate(categories):
        ...
        for x, pred_dict in enumerate(preds):
            ...
            if size == 0:
                size = 0.01
            circle = plt.Circle([x - size / 2, y - size / 2],
    ...
    for year in emb_dict_all.keys():
        ...
        for i, word in enumerate(test_words):
            ...
            if embedding is not None:
                test_df.append(
                    {constant.YEAR: year, constant.WORD: word, constant.VECTOR: embedding})
...
def run(args):
    ...
    emb_dict_all, _ = embeddings.choose_emb_dict(args.corpus, args.embeddings)
    nyt_corpus = args.corpus
    test_words = args.vars

    test_df = load_test_df(emb_dict_all, test_words)
    mfd_dict = models.load_mfd_df(args.data_dir, emb_dict=emb_dict_all)
    mfd_dict_binary = models.load_mfd_df_binary(args.data_dir, emb_dict=emb_dict_all)
    mfd_dict_null = models.load_mfd_df_neutral(args.data_dir, emb_dict=emb_dict_all)
    all_results = {}
    results = []
    for binary_fine_grained in ['NULL', 'BINARY', 'FINEGRAINED']:
        all_fitted_models = [set_model(c, args.bootstrap, binary_fine_grained,
            mfd_dict, mfd_dict_binary, mfd_dict_null)
                             for c in all_models]
        ...
        for model_list in all_fitted_models:
            ...
            for i, word in enumerate(test_df[constant.WORD].unique()):
                word_df = test_df[test_df[constant.WORD] ==
                                  word].sort_values(by=[constant.YEAR])
                years = word_df[constant.YEAR].values
                mean_line, lower_bound, upper_bound = models.models_predictions(
                    model_list, word_df, args.bootstrap)
        ...
        for binary_fine_grained in ['NULL', 'BINARY', 'FINEGRAINED']:
            ...
            if 'FINEGRAINED' == binary_fine_grained:
                if len(mean_line) < len(constant.ALL_YEARS):
                    mean_line = [
                        None] * (len(constant.ALL_YEARS) - len(mean_line)) + mean_line
                plt.subplot(313)
                ...
                plot_category_hinton(categories, mean_line,
                                     yticks_locs, max_weight, min_weight)
                ...
                plot_binary(word, mean_line, word, years, args.bootstrap, binary_fine_grained, ylim1, ylim2, min(all_years),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--corpus', default='NGRAM')
